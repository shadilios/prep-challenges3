...
const employeesBonus = (arr) => {
    // write your code here
    //I have to make a new array
    let newArr=arr;
    for(let i=0;i<newArr.length;i++){
      if(newArr[i].workHours>8){
        //newArr[i].salary +=100;
        newArr[i].salary =parseInt(newArr[i].salary.slice(0,newArr[i].salary.length-1))+100+"$";
      }else{
        newArr[i].salary =parseInt(newArr[i].salary.slice(0,newArr[i].salary.length-1))+50+"$";
      }
    }
    return newArr;
